...
import { Search, Sparkles, LogOut, User as UserIcon, Film, Tv, Gamepad2, BookOpen, Menu, X } from "lucide-react";
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const categories: { label: string; value: MediaType | "ALL"; icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-slate-800 bg-slate-950/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Media Categories Bar */}
        <div className="hidden md:flex items-center gap-1.5 pb-3 pt-1 border-t border-slate-900/60 overflow-x-auto no-scrollbar">
          {categories.map((cat) => {
            const Icon = cat.icon;
            ...
            <div className="pt-3 border-t border-slate-900">
              {user ? (
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-slate-300">Signed in as {user.username}</span>
